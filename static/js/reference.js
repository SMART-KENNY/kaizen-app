...
function shouldSkipCast(fieldName) {
  ...
  const noCastFields = ['msisdn', 'imsi', 'imei', 'file_name', 'file_id'];
...
function generateSelectStatement(fields, pipeLine, headerFlag) {
  const selectLines = fields.map((f, index) => {
    ...
    if (fieldNameLower === 'dbx_process_dttm') {
      return `  now() ${fieldNameLower}`;
    }
    ...
    if (headerFlag) {
      ...
      if (dataType === 'timestamp') {
        return `  to_timestamp(${fieldNameLower},'yyyy-MM-dd HH:mm:ss.SSS') ${fieldNameLower}`;
      }else if(dataType === 'date'){
        return `  to_date(${fieldNameLower},'yyyy-MM-dd') ${fieldNameLower}`;
      }
      else{
    ...
    // }
      ...
      if (dataType === 'timestamp') {
        return `  to_timestamp(${f.iterator},'yyyy-MM-dd HH:mm:ss.SSS') ${fieldNameLower}`;
      }else if(dataType === 'date'){
        return `  to_date(${f.iterator},'yyyy-MM-dd') ${fieldNameLower}`;
      }
      else{
        return `  cast(${f.iterator} as ${dataType}) as ${fieldNameLower}`;
      }
...
function generateFullConfig(fields) {
  ...
  const endpoint_configuration_tbl = `${workSpace}.${dataDomain}.${pipeLine}`
...
function generateYAMLConfig() {
  ...
  const taskKey = `${pipeline}_reference_task`;

  const jobName = `${workspace}_${pipeline}_reference_job`;
